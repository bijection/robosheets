...
import * as language from '../lib/lang'
...
const colname = c =>
    c
        .toString(26)
        .split('')
        .map(c => 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'['0123456789abcdefghijklmnop'.indexOf(c)])
        .join('')
...
const suggestion_color = '#08c773' //'hsl(134, 50%, 50%)'
const text_image_cache = {}
...
function draw_cell_text(row, col) {
    ...
    //     let h = blurlin(r, c * 5)
    //     if (h > 0.7) ctx.fillStyle = 'rgba(0,100,0,' + h + ')'
    //     else if (h > 0.6) ctx.fillStyle = 'rgba(120,100,0,' + h + ')'
    //     else if (h > 0.6) ctx.fillStyle = '#ddca92'
    ...
    function draw_squished_text() {
        let cropped_text = text.slice(
            -Math.floor((text.length * (cell_width - result_width)) / text_width)
        )
        ctx.textAlign = 'end'
        ctx.fillStyle = drawing_suggestion_text ? suggestion_color : '#222'
...
function insert_csv(data, row = 0, col = 0) {
    ...
    let region = [row, col, row + lines.length, col + Math.max(...lines.map(line => line.length))]
...
function sync_canvas_and_keygetter() {
    ...
    keygetter.style.width =
        (measure_text(keygetter.value).width + cell_horizontal_padding + 2) / SCALE
    ...
    function train(x0, fn) {
        for (var col = 0; col < numsigma[0].length; col++) {
            ...
            var result = minimize(function f(x) {
                return _.sum(numoutputs.map((k, i) => (fn([numsigma[i][col], ...x]) - k) ** 2))
            }, x0)
            // } catch (err) {
...
function auto_fill() {
    ...
    for (var i = 0; i < col_ids.length; i++) {
        ...
        let max_col_height = _.max(
            col_ids.slice(0, i).map(k => _.max(_.flatten(cols[k].map(e => +e.split(',')[0]))))
        )
...
let handle_keydown = e => {
    ...
    if (e.keyCode == 39 && (!is_typing() || keygetter.selectionEnd === keygetter.value.length))
        bump(0, 1)
...
function execute(action) {
    ;({ row, col, selected_row, selected_col, selected_end_row, selected_end_col } = action)
