...
import { CPosSet, PosSet, SubStrSet, ExtdSubStrSet, ConstStrSet, DAG } from '../lib/langsets'

import { TokenNames, TokenRegexesG, TokenStrings } from '../lib/tokens'
...
function generate_short_posSet_set(s, k) {
    ...
    endings[k] &&
        endings[k].forEach(e1 => {
            bseq.push([e1.t])

            endings[e1.begin] &&
                endings[e1.begin].forEach(e2 => {
                    bseq.push([e2.t, e1.t])
    ...
    beginnings[k] &&
        beginnings[k].forEach(b1 => {
            eseq.push([b1.t])

            beginnings[b1.end] &&
                beginnings[b1.end].forEach(b2 => {
                    eseq.push([b1.t, b2.t])
...
function generate_posSet_set(s, k) {
    ...
    reps.forEach(t => {
        ...
        match = null
        let lastIndex
        while ((match = tok.exec(s)) && match.index < k && match.index != lastIndex)
...
function generate_str_kevin(sigma, s) {
    ...
    edges.forEach(edge => {
        ...
        map[key] = generate_substring(sigma, sij).concat([new ConstStrSet(sij)])
    })
    ...
    function helper(node) {
        ...
        for (var output_range of cartesian_product(...output_ranges)) {
            ...
            for (var i = 1; last.length > 0 && i < output_range.length; i++) {
                ...
                cross(last, get_edge_frag(i, ...output_range[i])).forEach(([f1, f2]) => {
                    var int = intersect(f1, f2)
                    if (int) intersection.push(int)
                })
                last = intersection
...
function pos_set_bucket(x) {
    return x instanceof CPosSet ? 'C' : 'P' + x.pre_regexes.length + ':' + x.post_regexes.length
}
...
function generate_position(s, k) {
    ...
    // TODO: check for off-by-one
    var rr1 = _.flatten(
        _.range(0, k + 1).map(k1 =>
            matching_token_sequences(substring(s, k1, k - 1)).map(r1 => [k1, r1])
        )
    )

    var rr2 = _.flatten(
        _.range(k + 1, s.length + 1).map(k2 =>
            matching_token_sequences(substring(s, k, k2)).map(r2 => [k2, r2])
        )
    )
    ...
    for (var [[k1, r1], [k2, r2]] of cross(rr1, rr2)) {
        ...
        // TODO: check for off-by-one
        var c = _.findIndex(matches, m => m.index == k1 && m.index + m[0].length == k2)

        result.push(
            new PosSet(generate_regex(r1, s), generate_regex(r2, s), [c, -(matches.length - c + 1)])
        ) // TODO: check for off-by-one
    }
