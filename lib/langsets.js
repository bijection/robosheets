...
export class SubStrSet {
    ...
    *all() {
    ...
    size() {
        return (
            _.sumBy(this.start_positions, p => p.size()) *
...
export class DAG {
    ...
    *_sample_from(node) {
    ...
    sample() {
        ...
        var trace = s.value.map(k => sample(k).sample())
    ...
    *all() {
        for (let path of this.all_paths_after(this.source)) {
            for (let trace of cartesian_product(
                ...path.map(edge =>
                    _.flatten(this.map[JSON.stringify(edge)].map(s => Array.from(s.all())))
                )
            )) {
                yield new Concatenate(...trace)
            }
    ...
    _size_of_node(n) {
        ...
        return _.sumBy(this._all_edges_to(n), edge => {
            var np = edge[0]
            return this._size_of_node(np) * _.sumBy(this.map[JSON.stringify(edge)], k => k.size())
        })
    }
    size() {
    ...
    prune() {
        ...
        var edges = this.edges.filter(
            k => JSON.stringify(k[0]) in connected && JSON.stringify(k[1]) in connected
        )
